...
profile = {}
profile["permissions"] = (
    '{"20": [10, 11, 12, 13], "21": [10, 11, 12, 13], "22": [10, 11, 12, 13], "23": [10, 11, 12, 13], "24": [10, 11, 12, 13], "25": [10, 11, 12, 13], "26": [10, 11, 12, 13]}'
)
...
def generate_png(headers, rows):
    ...
    header_height = (
        font.getsize(max(headers, key=len))[1] + cell_padding * 2
    )  # Height of header row
    cell_widths = [
        max(font.getsize(str(row[i]))[0] for row in rows + [headers]) + cell_padding * 2
        for i in range(len(headers))
    ]  # Width of each column
    image_width = sum(cell_widths)  # Width of table
    image_height = (
        header_height + line_height * len(rows) + cell_padding * 2
    )  # Height of table
