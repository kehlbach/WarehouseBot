...
btn_add_user = InlineKeyboardButton(
    ...
    callback_data=cb.generic.new(
        state=Generic.CALLBACK_TO_MESSAGE_INIT, action=User.Create.NUMBER, data=""
    ),
)
            ...
            keyboard.add(
                InlineKeyboardButton(
                    ...
                    callback_data=cb.generic.new(
                        ...
                        data=dumps([cb_data["profile_id"], profile["phone_number"]]),
                    ),
                )
            )
        else:
...
def get_user_departments(
    ...
    cb_data = {"state": "", "page": page, "profile_id": profile["id"]}

    page_row = _get_pages(
        departments_page,
        cb.user_departments,
        dict(action=action_class.MENU, department_id="", phone_number="", **cb_data),
    )
    ...
    keyboard.add(
        InlineKeyboardButton(
            ...
            callback_data=cb.user_departments.new(
                action=action_class.ALL, department_id="", phone_number="", **cb_data
            ),
        )
    )
...
def get_user_roles(
    action_class: User.Create.Roles | User.Edit.Roles, page, profile_id, roles_page
):
    keyboard = InlineKeyboardMarkup()

    cb_data = {"state": Generic.CALLBACK_HANDLE, "page": page, "profile_id": profile_id}

    page_row = _get_pages(
        roles_page, cb.user_role, dict(action=action_class.MENU, role_id="", **cb_data)
    )
    ...
    for each in roles_page["results"]:
        keyboard.add(
            InlineKeyboardButton(
                each["repr"],
                callback_data=cb.user_role.new(
                    action=action_class.SPECIFIC, role_id=each["id"], **cb_data
                ),
            )
        )
...
def get_profiles(master: dict, profiles_page: dict, page: int):
    ...
    page_row = _get_pages(
        profiles_page,
        cb.menu_item,
        dict(state=Menu.CHOICE, action=Menu.PROFILES, page=page),
    )
    keyboard.add(
        InlineKeyboardButton(
    ...
    if set([VIEW, EDIT, DELETE]).intersection(permissions[PROFILES]):
        cb_edit_data = {"state": User.Edit.MENU, "action": User.Edit.MENU}
