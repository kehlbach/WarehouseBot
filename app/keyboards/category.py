...
def get_categories(
    master: dict, categories_page: dict, page: int
    ...
    page_row = _get_pages(
        categories_page,
        cb.menu_item,
        dict(state=Menu.CHOICE, action=Menu.CATEGORIES, page=page),
    )
    keyboard.add(
        InlineKeyboardButton(
    ...
    if set([VIEW, EDIT, DELETE]).intersection(permissions[CATEGORIES]):
        cb_edit_data = {"state": Category.Edit.MENU, "action": Category.Edit.MENU}
