...
def edit_product(master, product):
    ...
    if EDIT in permissions[PRODUCTS]:
        ...
        keyboard.add(InlineKeyboardButton("Rename", callback_data=name_cb))
        keyboard.add(
            InlineKeyboardButton("Change vendor code", callback_data=vendor_cb)
        )
        keyboard.add(InlineKeyboardButton("Change unit", callback_data=unit_cb))
        keyboard.add(
            InlineKeyboardButton(
                "Change category",
    ...
    if DELETE in permissions[PRODUCTS]:
        del_cb = cb.generic.new(
            state=Generic.CALLBACK_HANDLE, action=Product.Edit.DELETE, data=product_id
        )
        keyboard.add(InlineKeyboardButton("Delete product", callback_data=del_cb))
...
def get_product_categories(
    ...
    cb_data = {"state": Generic.CALLBACK_HANDLE, "page": page, "product_id": product_id}

    page_row = _get_pages(
        categories_page,
        cb.product_category,
        dict(action=action_class.MENU, category_id="", **cb_data),
    )
    ...
    for each in categories_page["results"]:
        keyboard.add(
            InlineKeyboardButton(
                each["repr"],
                callback_data=cb.product_category.new(
                    action=action_class.SPECIFIC, category_id=each["id"], **cb_data
                ),
            )
        )
