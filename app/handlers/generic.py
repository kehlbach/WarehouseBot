...
from app.data.states import Category, Department, Generic, Product, Receipt, Role, User
...
@dp.callback_query_handler(
    cb.generic.filter(state=Generic.CALLBACK_TO_MESSAGE_INIT), state="*"
)
async def generic_message_request(
    ...
    async with state.proxy() as data:
        ...
        return await callback_query.message.answer(
            text=get_text[action], reply_markup=reply_markup
        )
...
async def generic_message_handler(message: types.Message, state: FSMContext):
    ...
    get_data_processor = {
        ...
        User.Edit.NUMBER_OWN: lambda: number_preprocessor(
            message, data["source_number"]
        ),
        Department.Edit.LOCATION: lambda: (
            ("", True, "")
            if message.text == "Remove location"
    ...
    get_db_operation = {
        ...
        Category.Create.NAME: lambda: db.add(
            db.CATEGORIES, requester=message.chat.id, name=processed_data
        ),
        ...
        Department.Create.NAME: lambda: db.add(
            db.DEPARTMENTS, requester=message.chat.id, name=processed_data
        ),
        ...
        Role.Create.NAME: lambda: db.add(
            db.ROLES, requester=message.chat.id, name=processed_data
        ),
        Role.Edit.NAME: lambda: db.edit_patch(
            db.ROLES, data["role_id"], requester=message.chat.id, name=processed_data
        ),
    ...
    set_state_data = {
        Department.Create.NAME: lambda: state.set_data(
            {"department": db_response, **data}
        ),
        Product.Create.VENDOR_CODE: lambda: state.set_data(
            {"vendor_code": processed_data, **data}
        ),
        Product.Create.NAME: lambda: state.set_data(
            {"product_id": db_response["id"], **data}
        ),
    }
    ...
    get_text = {
        ...
        User.Edit.NUMBER: lambda: "Phone number changed to {}.".format(
            db_response["phone_number"]
        ),
        User.Edit.NUMBER_OWN: lambda: "Phone number changed to {}.".format(
            db_response["phone_number"]
        )
        + "\nNow you can log in with another Telegram account.",
        Category.Create.NAME: lambda: 'Category "{}" created.'.format(
            db_response["name"]
        ),
        Category.Edit.NAME: lambda: 'Category name changed to "{}".'.format(
            db_response["name"]
        ),
        Department.Create.NAME: lambda: "Enter department address:",
        Department.Create.LOCATION: lambda: 'Department "{}" created:'.format(
            data["department"]["name"]
        ),
        Department.Edit.NAME: lambda: 'Department name changed to "{}".'.format(
            db_response["name"]
        ),
        Department.Edit.LOCATION: lambda: 'Department address changed to "{}".:'.format(
            db_response["location"]
        ),
        Role.Create.NAME: lambda: "Now set permissions for the role.",
        Role.Edit.NAME: lambda: 'Role name changed to "{}".'.format(
            db_response["name"]
        ),
        ...
        Product.Edit.NAME: lambda: 'Product name changed to "{}".'.format(
            db_response["name"]
        ),
        Product.Edit.VENDOR_CODE: lambda: 'Product vendor code changed to "{}".'.format(
            db_response["vendor_code"]
        ),
        Product.Edit.UNIT: lambda: 'Product unit of measurement changed to "{}".'.format(
            db_response["units"]
        ),
        Receipt.Edit.NOTE: lambda: 'Note was changed to "{}".'.format(
            db_response["note"]
        ),
    }
    get_reply_markup = {
        ...
        User.Create.NUMBER: lambda: get_user_roles(
            User.Create.Roles, 1, db_response["id"], db.get_page(db.ROLES)
        ),
        ...
        Department.Create.LOCATION: lambda: get_back(
        ...
        Role.Create.NAME: lambda: get_role_permissions(
            Role.Create.Permissions, db_response
        ),
        Role.Edit.NAME: lambda: get_back(ROLES, data["role_id"]),
        Product.Create.UNIT: lambda: get_product_categories(
            Product.Create.Category, 1, db_response["id"], db.get_page(db.CATEGORIES)
        ),
        ...
        Receipt.Edit.NOTE: lambda: get_back(
            RECEIPTS, [db_response["to_department"], data["receipt_id"]]
        ),
    }
