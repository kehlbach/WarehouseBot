...
@dp.callback_query_handler(cb.role_permissions.filter(action=(Role.Edit.Permissions.MENU,
                                                    ...
                                                    Role.Edit.Permissions.BACK)), state='*')
async def init_role_permissions(callback_query: CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    if action == action_class.BACK:
        ...
        role = db.get(db.ROLES, role_id)
        reply_markup = get_role_permissions(
            action_class,
            role
        )
            ...
            match action:
                ...
                case action_class.SPECIFIC:
                    ...
                    if action_id in permissions[subject_id]:
                        role_permission = db.filter(db.ROLE_PERMISSIONS, role=role_id,subject=subject_id, action=action_id)
