...
from app.data.constants import (CATEGORIES, DEPARTMENTS, INVENTORY, PRODUCTS,
                                PROFILES, RECEIPTS, ROLES)
...
async def process_menu_choice(callback_query: CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    match callback_data['action']:
        case Menu.PROFILES:
            ...
            profiles_page = db.get_page(
                db.PROFILES, callback_data.get('page', 1))
            text = 'Select user'
        ...
        case Menu.ROLES:
            ...
            reply_markup = get_roles(
                master, roles_page, callback_data.get('page', 1))
        case Menu.INVENTORY:
            if not permissions[INVENTORY]:
                raise PermissionError
            departments_page = db.get_page(
                db.DEPARTMENTS, callback_data.get('page', 1))
        ...
        case Menu.PRODUCTS:
            if not permissions[PRODUCTS]:
                raise PermissionError
            products_page = db.get_page(
                db.PRODUCTS, callback_data.get('page', 1))
            text = 'Select product'
        ...
        case Menu.CATEGORIES:
            if not permissions[CATEGORIES]:
                raise PermissionError
            categories_page = db.get_page(
                db.CATEGORIES, callback_data.get('page', 1))
