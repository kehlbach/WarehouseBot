...
from app.keyboards.receipt import (
    get_receipts,
    kb_add_product,
    kb_back_to_receipts,
    kb_edit_receipt,
    kb_get_create_department,
    kb_get_types,
)
...
async def work_on_receipts(
    ...
    if callback_data["data"]:  # Specific department
        ...
        receipts_page = db.get_page(
            db.RECEIPTS, callback_data.get("page", 1), allowed_to=master["id"]
        )
...
async def create_receipt_type(
    ...
    if not master["departments"]:
        return await callback_query.answer(
            "Can't create receipt if no departments available"
        )
...
async def create_receipt_type(
    ...
    master = db.filter(db.PROFILES, user_id=user_id)
    receipt = db.add(
        db.RECEIPTS, made_by=master["id"], requester=callback_query.message.chat.id
    )
...
@dp.callback_query_handler(
    cb.receipt_department.filter(state=Receipt.Create.DEPARTMENT), state="*"
)
    ...
    match action:
        ...
        case Receipt.Create.TO_DEP:
            ...
            if int(department_id) == receipt["from_department"]:
                return await callback_query.answer(
                    "Source and destination departments can't be the same"
                )
            changed_receipt = db.edit_patch(
                db.RECEIPTS,
                receipt_id,
        ...
        case Receipt.Create.PRODUCT:
            ...
            async with state.proxy() as data:
                ...
                if len(callback_data["data"]) > 2:
                    ...
                    rp = db.filter(
                        db.RECEIPT_PRODUCTS, receipt=receipt_id, product=product_id
                    )
...
async def create_product_quantity(message: Message, state: FSMContext):
    ...
    if quantity.isdigit():
        ...
        if quantity != 0:
            ...
            if (
                "available" in data
                and data["available"] + quantity_old >= quantity
                or not ("available" in data)
            ):
                receipt_product = db.add(
                    db.RECEIPT_PRODUCTS,
                    receipt=receipt_id,
                    product=product_id,
                    quantity=quantity,
                ...
                if "non_field_errors" in receipt_product:
                    if (
                        receipt_product["non_field_errors"][0]
                        == "The fields receipt, product, price must make a unique set."
                    ):
                        db.delete(
                            db.RECEIPT_PRODUCTS, id=rp["id"], requester=message.chat.id
                        )
                        receipt_product = db.add(
                            db.RECEIPT_PRODUCTS,
                            receipt=receipt_id,
        ...
        for rp in rps:
            text += "\n{}: {} {}".format(
                rp["product_name"], rp["quantity"], rp["product_units"]
            )
        products_page = db.get_page(db.PRODUCTS)
...
async def create_note(message: Message, state: FSMContext):
    ...
    department = (
        ...
        if receipt["to_department"]
        else receipt["from_department"]
    )
    text = "Receipt created"
    reply_markup = kb_back_to_receipts(master, receipt_id, department)
    if message.text != "Skip":
        db.edit_patch(
            db.RECEIPTS, receipt_id, note=message.text, requester=message.chat.id
        )
    return await message.answer(text=text, reply_markup=reply_markup)
