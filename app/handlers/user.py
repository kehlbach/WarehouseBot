...
async def handle_user_edit_department(
    ...
    match callback_data["action"]:
        ...
        case User.Edit.Departments.SPECIFIC | User.Create.Departments.SPECIFIC:
            ...
            profile = db.edit_put(
                db.PROFILES, profile, requester=callback_query.message.chat.id
            )
            reply_markup = get_user_departments(
                action_class,
                page=page,
        ...
        case User.Create.Departments.DONE:
            text = "User with number {} created".format(callback_data["phone_number"])
            reply_markup = get_back(PROFILES, callback_data["profile_id"])
...
async def handle_user_edit_role(
    callback_query: CallbackQuery, callback_data: dict, state: FSMContext
):
    action = callback_data["action"]
    page = callback_data.get("page", 1)
...
async def edit_user_init(
    ...
    if set([VIEW, EDIT, DELETE]).intersection(permissions[PROFILES]):
        ...
        if master["id"] == profile["id"]:
            ...
            text = "User"
        all_departments_id_to_name = {i["id"]: i["name"] for i in all_departments}
        ...
        reply_markup = edit_user(master=master, profile=profile)
        return await bot.edit_message_text(
            chat_id=callback_query.message.chat.id,
            message_id=callback_query.message.message_id,
            text=text,
